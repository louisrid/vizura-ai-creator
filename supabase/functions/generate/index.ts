...
async function generateAngleAndBody(
  ...
  if (target === "angle" || target === "both") {
    try {
      ...
      const anglePrompt = `EXACT same person from reference photo with IDENTICAL hair colour, hair tone, and facial features — no shift in warmth, coolness, or saturation of hair. A ${characterTraits.includes('young-woman') ? 'young-woman' : 'woman'} with ${characterTraits}. ${bustDesc} with visible cleavage. Tight white v-neck top, same white background, same lighting. Head turned 45 degrees to the left showing 3/4 profile. Framed from top of head to stomach. MATTE skin with clearly visible pores, peach fuzz, subtle natural imperfections, realistic skin texture, NO glossy, NO shiny skin. Relaxed neutral expression, lips together. Hair colour and face must be identical to reference photo.`;
