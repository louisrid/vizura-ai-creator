      ...
      <main className="mx-auto flex w-full max-w-lg flex-col px-4 pt-28 pb-12">
        ...
        <section className="mt-10 flex flex-col gap-2">
          ...
          <textarea
            ...
            rows={4}
            className="min-h-32 w-full resize-none rounded-2xl border-[3px] border-border bg-card px-4 py-3 text-sm font-extrabold lowercase text-foreground placeholder:text-foreground/30 focus:border-foreground focus:outline-none transition-colors"
          />
        </section>
        ...
        {error && (
          <div className="mt-6 rounded-2xl border-[3px] border-destructive/30 bg-destructive/5 p-4 text-sm font-extrabold lowercase text-destructive">
            {error}
          </div>
        )}
...
const SelectField = <T extends string>({ label, options, value, onChange }: SelectFieldProps<T>) => (
  <label className="relative flex flex-col gap-2">
    ...
    <select
      ...
      className="h-12 w-full appearance-none rounded-2xl border-[3px] border-border bg-card px-4 pr-10 text-sm font-extrabold lowercase text-foreground outline-none transition-colors focus:border-foreground"
    >
      {options.map((option) => (
