      ...
      {/* Face picker — shown after loading completes */}
        {!loading && faces.length > 0 && (
          <>
            ...
            <main className="relative z-[1] mx-auto flex w-full max-w-lg flex-col overflow-y-auto px-[20px] pt-[42px] pb-[140px] md:max-w-3xl md:px-10">
                        ...
                        <div className="grid grid-cols-4 gap-1.5">
                          {traitItems.map((t) => (
                            <div key={t.label} className="rounded-[8px] py-2 text-center" style={{ backgroundColor: "hsl(var(--card))" }}>
                              ...
                              <span className={`inline-block font-[800] text-white leading-none text-[11px] border-[1px] border-[hsl(var(--border-mid))] rounded-[8px] ${t.value === "XL" ? "normal-case" : "lowercase"}`} style={{ backgroundColor: "hsl(var(--card))", padding: "6px 12px" }}>{t.value}</span>
                            </div>
                          ))}
                        </div>
                      </div>
