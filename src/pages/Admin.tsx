  ...
  <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/95 px-4" onClick={onClose}>
    ...
    <div className="max-w-md w-full" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="mt-4 space-y-1.5">
        <p className="text-[12px] font-extrabold lowercase text-white leading-snug">{photo.prompt || "no prompt"}</p>
        <p className="text-[10px] font-extrabold lowercase" style={{ color: "rgba(255,255,255,0.4)" }}>
          {photo.user_email}
...
const Admin = () => {
  ...
  return (
    <div className="relative min-h-screen bg-background overflow-hidden">
      ...
      <main className="relative z-[1] w-full max-w-lg mx-auto px-[14px] pt-10 pb-20">
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-2.5 mb-8">
              {[
                ...
                <div key={label} className="flex flex-col items-center justify-center py-4" style={{ borderRadius: 14, backgroundColor: "#1a1a1a" }}>
                  <Icon size={16} strokeWidth={2.5} style={{ color: "#facc15" }} className="mb-1" />
                  <span className="text-[28px] font-[900] text-white leading-none">{value.toLocaleString()}</span>
                  <span className="text-[9px] font-extrabold lowercase mt-1" style={{ color: "rgba(255,255,255,0.35)" }}>{label}</span>
                </div>
              ))}
            </div>
            ...
            {/* LATEST PHOTOS */}
            <div className="mb-8">
              <h2 className="text-sm font-[900] lowercase text-white mb-3">latest photos</h2>
              {photos.length > 0 ? (
                <div className="grid grid-cols-2 gap-2.5">
                  {photos.map((p: any, i: number) => (
                    <button key={i} onClick={() => setSelectedPhoto(p)} className="text-left" style={{ borderRadius: 14, backgroundColor: "#1a1a1a", overflow: "hidden" }}>
                      <img src={p.image_url} alt="" className="w-full aspect-[3/4] object-cover" />
                      <div className="px-3 py-2.5">
                        <p className="text-[10px] font-extrabold lowercase text-white truncate leading-tight">{p.prompt || "—"}</p>
                        <p className="text-[9px] font-extrabold lowercase mt-0.5 truncate" style={{ color: "rgba(255,255,255,0.3)" }}>{p.user_email}</p>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {/* ACTIVE USERS */}
            <div className="mb-8">
              <h2 className="text-sm font-[900] lowercase text-white mb-3">active users (7d)</h2>
              {activeUsers.length > 0 ? (
                <div className="space-y-1.5">
                  {activeUsers.map((u: any, i: number) => (
                    <div key={i} className="flex items-center justify-between px-3.5 py-2.5" style={{ borderRadius: 12, backgroundColor: "#1a1a1a" }}>
                      <span className="text-[11px] font-extrabold lowercase text-white truncate flex-1 mr-3">{u.email}</span>
                      <div className="flex items-center gap-3 shrink-0">
                        <span className="text-[10px] font-extrabold lowercase" style={{ color: "rgba(255,255,255,0.5)" }}>{u.photos_this_week} photos</span>
                        <span className="text-[10px] font-extrabold lowercase" style={{ color: "#00e0ff" }}>{u.gems_remaining}💎</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* FLAGGED */}
            <div className="mb-8">
              <h2 className="text-sm font-[900] lowercase text-white mb-3">flagged</h2>
              {rejected.length > 0 ? (
                <div className="space-y-1.5">
                  {rejected.map((r: any, i: number) => (
                    <div key={r.id || i} className="px-3.5 py-2.5" style={{ borderRadius: 12, backgroundColor: "#1a1a1a" }}>
                      <p className="text-[11px] font-extrabold lowercase text-white leading-snug">{r.prompt_text}</p>
