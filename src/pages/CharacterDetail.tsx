  ...
  if (loading || (authLoading && !hasCachedUser)) {
    return (
      <div className="relative min-h-screen overflow-hidden">
        <main className="relative z-[1] mx-auto w-full max-w-lg md:max-w-3xl px-[24px] md:px-[48px] pt-[34px] pb-[200px]">
          ...
          <div style={{ backgroundColor: "hsl(var(--card))", borderRadius: 8 }} className="p-5">
            {editingName ? (
              <div className="flex items-center gap-2 mb-5">
                <Input ref={nameInputRef} value={editName} onChange={(e) => setEditName(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") saveEditName(); if (e.key === "Escape") { setEditingName(false); } }} className="flex-1 font-[900] lowercase text-white px-3 py-0" style={{ fontSize: 30, height: 52, backgroundColor: "hsl(var(--card))", border: "1.75px solid hsl(var(--border-mid))", borderRadius: 8 }} />
            ...
            <div className="grid grid-cols-4 gap-1">
              {traits.map((t) => (
                <div key={t.label} className="rounded-[8px] py-2 text-center" style={{ backgroundColor: "hsl(var(--card))" }}>
                  ...
                  <span className={`inline-block font-[800] text-white leading-none text-[10px] border-[1.75px] border-[hsl(var(--border-mid))] rounded-[8px] ${t.value === "XL" ? "normal-case" : "lowercase"}`} style={{ backgroundColor: "hsl(var(--card))", padding: "4px 8px" }}>{t.value}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={{ backgroundColor: "hsl(var(--card))", borderRadius: 8 }} className="p-5">
              <h3 className="text-sm font-[900] lowercase text-white mb-3">traits</h3>
               <div className="grid grid-cols-2 gap-2">
                {traits.map((t) => (
                  <div key={t.label} className="rounded-[8px] px-3 py-2 text-center" style={{ backgroundColor: "hsl(var(--card))", border: "1.75px solid hsl(var(--border-mid))" }}>
        ...
        {showDelete && (
          ...
          >
            <motion.div
              ...
              style={{ backgroundColor: "hsl(var(--card))", borderRadius: 8, border: "1.75px solid hsl(var(--border-mid))", padding: "28px 24px 24px" }}
            >
