      ...
      <main className="relative z-[1] w-full max-w-lg md:max-w-6xl mx-auto px-[24px] md:px-[44px] pt-[44px] pb-[140px]">
        ...
        {items.length === 0 ? (
          <div className="rounded-[6px] p-8 md:p-12 text-center md:max-w-md md:mx-auto" style={{ backgroundColor: "hsl(var(--card))", border: "2px solid hsl(var(--border-mid))" }}>
            ...
            <p className="text-xs md:text-sm font-semibold lowercase mb-4 text-foreground">no photos yet</p>
            <button
              onClick={() => navigate("/create")}
              className="h-12 md:h-14 w-full max-w-[12rem] mx-auto flex items-center justify-center gap-2 bg-neon-yellow text-lg md:text-xl font-semibold lowercase text-neon-yellow-foreground hover:opacity-90 transition-all"
          ...
          <div className="space-y-4 md:grid md:grid-cols-3 lg:grid-cols-4 md:gap-5 md:space-y-0">
            {items.map((item) => (
              <button
                ...
                className="w-full text-left rounded-[6px] overflow-hidden transition-all hover:border-foreground/60 hover-lift"
              ...
              >
                ...
                <div className="p-4 space-y-2">
                  <p className="text-[11px] md:text-[12px] font-semibold lowercase text-white line-clamp-2 leading-relaxed">
                    {item.prompt || "no prompt"}
                  </p>
                  <div className="flex items-center justify-between">
                    {item.characterName && (
                      <div className="flex items-center gap-1.5 text-[10px] md:text-[11px] font-semibold lowercase text-neon-yellow">
                        <User size={10} strokeWidth={2.5} />
                        {item.characterName}
                      </div>
                    )}
                    <div className={`flex items-center gap-1 text-[10px] font-semibold lowercase text-white ${!item.characterName ? "ml-auto" : ""}`}>
