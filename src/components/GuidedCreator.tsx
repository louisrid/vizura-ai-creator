  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex flex-col overflow-hidden" style={{ background: "#000" }}>
      ...
      >
        ...
        {/* Content area */}
        <div className="absolute inset-0 flex items-center justify-center px-6 md:px-12 md:top-16 md:bottom-40" style={{ paddingBottom: isHeroSlide ? "0%" : "0%", paddingTop: isHeroSlide ? "5%" : "0%" }}>
                ...
                <div className="flex items-center justify-center gap-[3px] md:gap-[5px] w-full max-w-[280px] md:max-w-sm mx-auto">
                  {Array.from({ length: dashCount }).map((_, i) => (
                    <div key={i} className="transition-all duration-300 h-[4px] md:h-[6px]" style={{
                      ...
                      background: i < activeIndex ? Y : i === activeIndex ? Y : "rgba(250,204,21,0.1)",
                    }} />
                  ))}
                </div>
              );
