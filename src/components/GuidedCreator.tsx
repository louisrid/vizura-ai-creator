...
const NavArrow = ({ direction, onClick, disabled, colorOverride }: { direction: "left" | "right"; onClick: () => void; disabled?: boolean; colorOverride?: string }) => {
    ...
    const fillColor = colorOverride || "#dddddd";
      ...
      <div className="flex flex-col items-center px-8 w-full max-w-md">
        ...
        <div className="mt-8 w-full rounded-[2px] border-2 border-[hsl(var(--border-mid))] p-5 md:p-8 space-y-3 md:space-y-4" style={{ backgroundColor: "hsl(var(--card))" }}>
          ...
          <button
            ...
            style={{ borderRadius: 2, background: '#dddddd', color: '#000000' }}
          >
  ...
  const renderHero = () => {
    ...
    return (
      <div className="flex w-full flex-col items-center justify-center" style={{ minHeight: '100%', paddingTop: '6vh', paddingBottom: '2vh' }}>
        ...
        <div style={{ width: 204, height: 16, background: '#dddddd', borderRadius: 2, marginTop: 6, marginBottom: 28, opacity: heroPhase >= 3 ? 1 : 0, transition: 'opacity 0.9s ease' }} />
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, opacity: heroPhase >= 3 ? 1 : 0, transition: 'opacity 0.9s ease' }}>
          <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); advance(); }} style={{ width: 204, padding: '10px 0', fontSize: 32, fontWeight: 900, background: '#dddddd', border: 'none', borderRadius: 2, color: '#000000', textTransform: 'lowercase', cursor: 'pointer', letterSpacing: '-0.02em' }}>start</button>
          {(
            ...
            }} style={{ width: 204, padding: '8px 0', fontSize: 32, fontWeight: 900, background: '#000000', border: '2px solid #dddddd', borderRadius: 2, color: '#ffffff', textTransform: 'lowercase', cursor: 'pointer', letterSpacing: '-0.02em' }}>login</button>
          )}
        </div>
      </div>
    );
